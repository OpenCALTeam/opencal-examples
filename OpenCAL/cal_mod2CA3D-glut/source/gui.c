...
void simulationRun(void)
{
  ...
  //exectutes the global transition function, the steering function and check for the stop condition.
    ...
    if (!again)
    {
        ...
        //graphic rendering
        if(mod2->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS)
            printf("step: %d; \tactive cells: %d\r", mod2_simulation->step, mod2_simulation->ca3D->contiguousLinkedList->size_current);
        else if(mod2->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            printf("step: %d; \tactive cells: %d\r", mod2_simulation->step, mod2_simulation->ca3D->A->cells);
        else
            printf("step: %d; \tactive cells: %d\r", mod2_simulation->step, 0);
        glutPostRedisplay();
        return;
    }
