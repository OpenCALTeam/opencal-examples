...
void simulationRun(void)
{
  ...
  //exectutes the global transition function, the steering function and check for the stop condition.
    ...
    if (!again)
    {
        ...
        else if(life3D->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            printf("step: %d; \tactive cells: %d\r", life3Dsimulation->step, life3Dsimulation->ca3D->A->cells);
        else
            printf("step: %d; \tactive cells: %d\r", life3Dsimulation->step, 0);
        glutPostRedisplay();
        return;
    }
