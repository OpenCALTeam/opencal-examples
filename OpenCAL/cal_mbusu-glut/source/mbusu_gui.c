...
void simulationRun(void)
{
    ...
    if (!again)
    {
        ...
        //graphic rendering
        if(mbusu->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS)
            printf("step: %d; \tactive cells: %d\r", mbusuSimulation->step, mbusuSimulation->ca3D->contiguousLinkedList->size_current);
        else if(mbusu->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            printf("step: %d; \tactive cells: %d\r", mbusuSimulation->step, mbusuSimulation->ca3D->A->cells);
        else
