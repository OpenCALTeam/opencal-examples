...
int main(){
    ...
    for(int i = 0; i < numberofWell; i++){
        ...
        name_with_extension = (char*) malloc(strlen(name)+1+2); /* make space for the new string (should check the return value ...) */
    ...
    double * lastModFlow;
    int rowsModFlow = 648000;
    int colsModFlow = 10;
    lastModFlow = (double*) malloc(sizeof(double)*(rowsModFlow*colsModFlow));

    calLoadMatrix2Dr(lastModFlow, rowsModFlow, colsModFlow, "./prova_river2.fhd");

    int numberofMatrix = 648; // this variables identies the number of computational steps example (12*86400)/4000 = 259
    ...
    for(int k = 0;k< numberofMatrix;k++){
        ...
        {
            for(int i = 0; i < numberofWell; i++){
                fprintf(monitorWellFiles[i], "%d %f \n", days, lastModFlowCorretta[k*ROWS*COLS + (rowMonitoringWell[i]*COLS+colMonitoringWell[i])]);
            }
            numberofdays++;
        }
    ...
    for(int i = 0; i < numberofWell; i++){
                fprintf(monitorWellFiles[i], "%d %f \n", 30, lastModFlowCorretta[k*ROWS*COLS + (rowMonitoringWell[i]*COLS+colMonitoringWell[i])]);
    }
    ...
    int indexJ = 0;
     k = 180;
        for(int i = 0; i< ROWS;i++){
            for(int j = 0; j< COLS;j++){
                if(lastModFlowCorretta[k*ROWS*COLS + (i*COLS+j)] < min){
                    indexJ = j;
                    min = lastModFlowCorretta[k*ROWS*COLS + (i*COLS+j)] ;
                }
