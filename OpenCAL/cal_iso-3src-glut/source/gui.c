  ...
  //exectutes the global transition function, the steering function and check for the stop condition.
    ...
    if (!again)
    {
       ...
       // printf("Saving final state to %s\n", OUTPUT_PATH);
        ...
        //graphic rendering
        if(ca[currentAC].iso->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS)
            printf("step: %d; \tactive cells: %d\r", ca[currentAC].isoRun->step, ca[currentAC].isoRun->ca2D->contiguousLinkedList->size_current);
        else if(ca[currentAC].iso->OPTIMIZATION == CAL_OPT_ACTIVE_CELLS_NAIVE)
            printf("step: %d; \tactive cells: %d\r", ca[currentAC].isoRun->step, ca[currentAC].isoRun->ca2D->A->cells);
        else
            printf("step: %d; \tactive cells: %d\r", ca[currentAC].isoRun->step, 0);
        glutPostRedisplay();
        return;
    }
